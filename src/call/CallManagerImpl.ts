    ...
    CallTimeoutListener
{
  ...
  public acceptCall(params: {
    ...
    calllog.log('CallManagerImpl:acceptCall:11111', params);
    const call = this._getCall(params.callId);
        calllog.log('CallManagerImpl:acceptCall:2222',call);

    if (call) {
      if (call.isInviter === false) {

        const invitee = call.invitees.get(this.userId);
    calllog.log('CallManagerImpl:acceptCall:3333',call ,invitee);
        ...
        // if (invitee) {
          ...
          this.signalling.sendInviteReply({
            callId: call.callId,
            inviterId: call.inviter.userId,
            inviteeDeviceToken:invitee ? invitee.userDeviceToken ?? '' : '',
            inviterDeviceToken: call.inviter.userDeviceToken ?? '',
            reply: 'accept',
  ...
  public joinChannel(params: {
    ...
    const ret = this.engine?.joinChannel(
      params.userRTCToken,
      params.channelId,
      Number(params.userChannelId),
      {
        clientRoleType: ClientRoleType.ClientRoleBroadcaster,
      }
    );
    calllog.log('CallManagerImpl:joinChannel:ret:', params,Number(params.userChannelId), ret);
  }
            ...
            if (p.error === undefined && p.data?.result) {
              ...
              Object.entries(p.data.result).forEach((value: [string, any]) => {
                ...
                if (call.inviter.userId === value[1]) {
                                calllog.log('CallManagerImpl:onUserJoined:requestUserMap:11111', value);

                  call.inviter.userChannelId = Number(value[0]);
                  call.inviter.userHadJoined = true;
                } else {
                                calllog.log('CallManagerImpl:onUserJoined:requestUserMap:2222', value);
              ...
              calllog.log(
                'CallManagerImpl:onUserJoined:requestUserMap3333:',
                call.inviter,
                call.invitees
              );
